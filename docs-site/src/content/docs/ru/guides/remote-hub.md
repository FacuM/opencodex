...
```bash
# Подойдёт любой loopback-форвардер TCP; socat — один из них. Слушатель данных привязан
# к tailnet-адресу, поэтому 127.0.0.1:10100 свободен и его может занять форвардер.
...
tailscale serve status   # ожидаются оба сопоставления: 443 -> 10101 и 8443 -> 10100
```
